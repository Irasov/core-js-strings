...
function removeFirstOccurrences(str, value) {
  ...
  if (search !== -1) {
    return str.slice(0, search) + str.slice(search + value.length);
  }
  return str;
}
...
function removeLastOccurrences(str, value) {
  const search = str.lastIndexOf(value);
  if (search !== -1) {
    return str.slice(0, search) + str.slice(search + value.length);
  }
  return str;
...
function sumOfCodes(str) {
  let sum = 0;
  if (str === '' || str === undefined || str === null) return sum;
...
function countVowels(str) {
  const alph = ['a', 'e', 'i', 'o', 'u', 'y', 'A', 'E', 'I', 'O', 'U', 'Y'];
  let count = 0;
  for (let i = 0; i < alph.length; i += 1) {
    for (let j = 0; j < str.length; j += 1) {
      if (alph[i] === str[j]) count += 1;
    }
  }
  return count;
}
...
function invertCase(str) {
  ...
  for (let i = 0; i < letters.length; i += 1) {
    if (letters[i] === str[i]) letters[i] = letters[i].toLowerCase();
  }
  return letters.join('');
}
